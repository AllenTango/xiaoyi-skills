...
| `buildCtx(config)` | `(c: Config) => Ctx` | Build the adapter ctx (SITE_ROOT / PIPELINE_DIR / CACHE_DIR / config / fetchWithCache / applyMap / log). Called once per `build()`; the returned object is shared across all adapters in that build. |
| `applyDerivedGroupBy(items, field)` | `(items, field) => { slug, value, count, items }[]` | Distinct values of `items[i][field]`; each group has the slugified value as `slug`, the original value as `value`, the count, and the items that matched. Slugs use the same slug rules as items (see `data-sources.md` §Slug Rules). |
| `applyDerivedFilter(items, where)` | `(items, where) => items` | Same predicate as `filterWhere`; sourced via the `derived` `op: "filter"` view. |
| `applyDerivedMap(items, fields)` | `(items, fields) => items` | Keep only the named fields (or `def.map` keys) on each item. Drop everything else. |
| `applyDerivedFlatten(items, field)` | `(items, field) => items` | Items where `items[i][field]` is an array; emit one item per array element (carrying through the parent item's other fields). Empty / non-array field → emit a single item with the field cleared. |
| `applyDerivedUnique(items, field)` | `(items, field) => items` | First occurrence wins. Comparison is `===` on the field value. Items without the field are dropped. |
| `applyDerivedCount(items)` | `(items) => { count }[]` | Wrap the count as a single-item array: `[{ count: items.length }]`. Use as a derived source so a `for.each` view over it produces a single stat page. |
| `computePageMeta(task, site)` | `(task, site) => { pageTitle, pageDescription }` | Derive the top-level `pageTitle` / `pageDescription` injected into template data (see `templates/conventions.md` §3). For `for.each`: `pageTitle = item.title`, `pageDescription = item.summary || item.excerpt || site.description`. For `for.paginate` / single: `pageTitle = site.title` (paginate pages have no item-specific title unless `view.data.pageTitle` is declared). The values are exposed at the **top level** of the per-task `data` object, not inside `task.data`. |
...
```js
{
  ...
  fetchWithCache,                  // (url, init, cacheKey, ttl) => Promise<Response>
                                   //   - MUST scrub every resolved `auth.env` value from the
                                   //     response body and any error message before returning,
                                   //     before writing the snapshot, and before logging
                                   //   - MUST NOT echo the secret in the cache filename
                                   //     (cacheKey is the user-supplied `cache.key` or a
                                   //     sha256 of the source def MINUS secrets — see
                                   //     data-sources.md §Snapshot Cache)
  applyMap,                        // (item, def.map, def.defaults) => Item
  log,                             // (level, message, meta?) => void  (console fallback; never logs raw secret values)
  name,                            // current source name
  datasets,                        // already-resolved datasets (for derived adapters)
  resolvedSecrets,                 // { [envName]: value } of secrets actually used in this build
                                   //   (missing env → entry absent, NOT `undefined`)
}
```
...
export async function build(fresh = false) {
  ...
  for (const task of tasks) {
    ...
    const meta = computePageMeta(task, config.site);
    const data = { ...globals, ...task.data, ...meta };
