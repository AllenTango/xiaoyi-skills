...
│   ├── markdown.js           # default: local source/<name>/*.md
...
Full spec: see [`prompts/geo-conventions.md`](./geo-conventions.md). Source discipline: existing `source/<type>/*.md` IS the GEO source — do NOT add a `geo/` directory.
...
10. **User source directory names**: markdown content directories use `source/<name>` and media uses `source/media`; never generate leading-underscore subdirectories under `source/`.
