...
2. **脚本只读**：`scripts/xiaoyi-wsman-scan.js` 必须是只读的。任何修改文件的工作由 AI 用 Edit/Write 工具完成，不通过脚本。
...
| `scripts/xiaoyi-wsman-scan.js` | Node.js 18+ ESM 扫描校验脚本（跨平台） | 新增校验规则或输出格式变更 |
| `scripts/package.json` | 声明 `"type": "module"` 使 .js 解析为 ESM | 极少修改 |
...
2. 在 `xiaoyi-wsman-scan.js` 的注释中登记规则名（机读视角）。
3. 实现脚本逻辑：先检查 STATUS.md 存在再访问 frontmatter；issue 描述清晰、可读；JSON 输出字段顺序保持一致。
...
4. **JSON 输出一致性**：`--json` 输出字段顺序固定为 `name / stage / progress / last_updated / reviewed / tested / git / issues`。
5. **环境变量**：`WSMAN_STALE_DAYS` 自定义值生效。
